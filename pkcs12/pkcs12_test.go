...
func Test_P12Encrypt(t *testing.T) {
	...
	x, _ := x509.ParseCertificate(decodeBytes)
	...
	SM2P12Encrypt(x, "123", privKey.(*sm2.PrivateKey), "test.p12") //根据证书与私钥生成带密码的p12证书
...
func Test_P12Dncrypt(t *testing.T) {
	...
	sm2Priv := privatekey.(*sm2.PrivateKey)
	fmt.Println(certificate.Issuer)
	fmt.Println(sm2Priv.D.Cmp(priv.D) == 0)

...
func printSubjectInformation(subj *pkix.Name, pkAlgo PublicKeyAlgorithm, pk interface{}, buf *bytes.Buffer) error {
...
func printSignature(sigAlgo SignatureAlgorithm, sig []byte, buf *bytes.Buffer) {
		...
		for _, ext := range cert.Extensions {
			if len(ext.Id) == 4 && ext.Id[0] == 2 && ext.Id[1] == 5 && ext.Id[2] == 29 {
				...
				case 15:
					...
					if cert.KeyUsage&KeyUsageContentCommitment > 0 {
						usages = append(usages, "Content Commitment")
					}
					if cert.KeyUsage&KeyUsageKeyEncipherment > 0 {
						usages = append(usages, "Key Encipherment")
					}
					if cert.KeyUsage&KeyUsageDataEncipherment > 0 {
						usages = append(usages, "Data Encipherment")
					}
					if cert.KeyUsage&KeyUsageKeyAgreement > 0 {
					...
					if cert.KeyUsage&KeyUsageEncipherOnly > 0 {
						usages = append(usages, "Encipher Only")
					}
					if cert.KeyUsage&KeyUsageDecipherOnly > 0 {
				...
				var rawSCT []byte
				rest, err := asn1.Unmarshal(ext.Value, &rawSCT)
				if err != nil {
					return "", fmt.Errorf("certinfo: Error parsing SCT %s, err: %s", ext.Id.String(), err.Error())
				}
				if len(rest) > 0 {
				...
				var sctList SignedCertificateTimestampList
				rest, err = UnmarshalSCT(rawSCT, &sctList)
				if err != nil {
					return "", fmt.Errorf("certinfo: Error parsing SCT %s, err: %s", ext.Id.String(), err.Error())
				}
				if len(rest) > 0 {

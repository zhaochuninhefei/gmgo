...
func TestServeWithSlashRedirectKeepsQueryString(t *testing.T) {
	...
	writeBackQuery := func(w ResponseWriter, r *Request) {
		_, _ = fmt.Fprintf(w, "%s", r.URL.RawQuery)
	}
	...
	mux.HandleFunc("/testThree/", func(w ResponseWriter, r *Request) {
		_, _ = fmt.Fprintf(w, "%s:bar", r.URL.RawQuery)
	})
...
func testServerTimeouts(timeout time.Duration) error {
	...
	ts := httptest.NewUnstartedServer(HandlerFunc(func(res ResponseWriter, req *Request) {
		reqNum++
		_, _ = fmt.Fprintf(res, "req=%d", reqNum)
	}))
...
func TestKeepAliveFinalChunkWithEOF(t *testing.T) {
	...
	cst := newClientServerTest(t, false /* h1 */, HandlerFunc(func(w ResponseWriter, r *Request) {
		...
		_, _ = w.Write([]byte("{\"Addr\": \"" + r.RemoteAddr + "\"}"))
	}))
	defer cst.close()
	type data struct {
		Addr string
...
func TestServerAllowsBlockingRemoteAddr(t *testing.T) {
	...
	ts := httptest.NewUnstartedServer(HandlerFunc(func(w ResponseWriter, r *Request) {
		_, _ = fmt.Fprintf(w, "RA:%s", r.RemoteAddr)
	}))
...
func testAutomaticHTTP2_Serve(t *testing.T, tlsConf *tls.Config, wantH2 bool) {
	...
	defer afterTest(t)
	ln := newLocalListener(t)
	_ = ln.Close() // immediately (not a defer!)
	var s Server
	s.TLSConfig = tlsConf
...
func TestRequestBodyReadErrorClosesConnection(t *testing.T) {
	...
	for _, handler := range testHandlerBodyConsumers {
		...
		var numReqs int
		go func() {
			_ = Serve(ls, HandlerFunc(func(_ ResponseWriter, req *Request) {
				numReqs++
...
func TestInvalidTrailerClosesConnection(t *testing.T) {
	...
	for _, handler := range testHandlerBodyConsumers {
		...
		var numReqs int
		go func() {
			_ = Serve(ln, HandlerFunc(func(_ ResponseWriter, req *Request) {
				numReqs++
				if strings.Contains(req.URL.Path, "secret") {
...
func testTransportAndServerSharedBodyRace(t *testing.T, h2 bool) {
	...
	defer func() {
		...
		quitTimer = time.AfterFunc(7*time.Second, func() {
			...
			_, _ = fmt.Fprintf(os.Stderr, "%s", stacks)
			log.Fatalf("Timeout.")
		})
	}()
...
func TestServerConnState(t *testing.T) {
	...
	wantLog(func() {
		...
		_, _ = c.Read(make([]byte, 1)) // block until server hangs up on us
		_ = c.Close()
...
func TestTolerateCRLFBeforeRequestLine(t *testing.T) {
	...
	numReq := 0
	go func() {
		_ = Serve(ln, HandlerFunc(func(rw ResponseWriter, r *Request) {
			numReq++
		}))
	}()
	<-conn.closec
...
`)
	...
	ln := &oneConnListener{conn: conn}
	go func() {
		_ = Serve(ln, HandlerFunc(func(w ResponseWriter, r *Request) {
...
func TestServerValidatesHostHeader(t *testing.T) {
	...
	for _, tt := range tests {
		...
		_, _ = io.WriteString(&conn.readBuf, methodTarget+tt.proto+"\r\n"+tt.host+"\r\n")
...
func TestServerHandlersCanHandleH2PRI(t *testing.T) {
	...
	ts := httptest.NewServer(HandlerFunc(func(w ResponseWriter, r *Request) {
		...
		_, _ = io.WriteString(conn, upgradeResponse)
	}))
	defer ts.Close()
	...
	_, _ = io.WriteString(c, "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n")
...
func TestServerValidatesHeaders(t *testing.T) {
	...
	for _, tt := range tests {
		conn := &testConn{closec: make(chan bool, 1)}
		_, _ = io.WriteString(&conn.readBuf, "GET / HTTP/1.1\r\nHost: foo\r\n"+tt.header+"\r\n")

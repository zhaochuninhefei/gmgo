...
#define hlp BP
/* ---------------------------------------*/
TEXT gmP256SubInternal(SB),NOSPLIT,$0
	XORQ mul0, mul0
	SUBQ t0, acc4
	SBBQ t1, acc5
	SBBQ t2, acc6
	...
	RET
/* ---------------------------------------*/
TEXT gmP256MulInternal(SB),NOSPLIT,$8
	MOVQ acc4, mul0
	MULQ t0
	MOVQ mul0, acc0
	MOVQ mul1, acc1
	...
	RET
/* ---------------------------------------*/
TEXT gmP256SqrInternal(SB),NOSPLIT,$8
...
TEXT ·p256PointAddAffineAsm(SB),0,$512-48
	...
	// Begin point add
	LDacc (z1in)
	CALL gmP256SqrInternal(SB)	// z1ˆ2
	ST (z1sqr)

	LDt (x2in)
	CALL gmP256MulInternal(SB)	// x2 * z1ˆ2

	LDt (x1in)
	CALL gmP256SubInternal(SB)	// h = u2 - u1
	ST (h)

	LDt (z1in)
	CALL gmP256MulInternal(SB)	// z3 = h * z1
	ST (zout)

	LDacc (z1sqr)
	CALL gmP256MulInternal(SB)	// z1ˆ3

	LDt (y2in)
	CALL gmP256MulInternal(SB)	// s2 = y2 * z1ˆ3
	ST (s2)

	LDt (y1in)
	CALL gmP256SubInternal(SB)	// r = s2 - s1
	ST (r)

	CALL gmP256SqrInternal(SB)	// rsqr = rˆ2
	ST (rsqr)

	LDacc (h)
	CALL gmP256SqrInternal(SB)	// hsqr = hˆ2
	ST (hsqr)

	LDt (h)
	CALL gmP256MulInternal(SB)	// hcub = hˆ3
	ST (hcub)

	LDt (y1in)
	CALL gmP256MulInternal(SB)	// y1 * hˆ3
	ST (s2)

	LDacc (x1in)
	LDt (hsqr)
	CALL gmP256MulInternal(SB)	// u1 * hˆ2
	ST (h)

	p256MulBy2Inline			// u1 * hˆ2 * 2, inline
	LDacc (rsqr)
	CALL gmP256SubInternal(SB)	// rˆ2 - u1 * hˆ2 * 2

	LDt (hcub)
	CALL gmP256SubInternal(SB)
	ST (xout)
...
// gmP256IsZero returns 1 in AX if [acc4..acc7] represents zero and zero
// otherwise. It writes to [acc4..acc7], t0 and t1.
TEXT gmP256IsZero(SB),NOSPLIT,$0
...
TEXT ·p256PointAddAsm(SB),0,$680-32
	...
	// Begin point add
	LDacc (z2in)
	CALL gmP256SqrInternal(SB)	// z2ˆ2
	ST (z2sqr)
	LDt (z2in)
	CALL gmP256MulInternal(SB)	// z2ˆ3
	LDt (y1in)
	CALL gmP256MulInternal(SB)	// s1 = z2ˆ3*y1
	ST (s1)
	...
	CALL gmP256MulInternal(SB)	// z1ˆ3
	LDt (y2in)
	CALL gmP256MulInternal(SB)	// s2 = z1ˆ3*y2
	ST (s2)

	LDt (s1)
	CALL gmP256SubInternal(SB)	// r = s2 - s1
	ST (r)
	CALL gmP256IsZero(SB)
	MOVQ AX, points_eq

	LDacc (z2sqr)
	LDt (x1in)
	CALL gmP256MulInternal(SB)	// u1 = x1 * z2ˆ2
	ST (u1)
	LDacc (z1sqr)
	LDt (x2in)
	CALL gmP256MulInternal(SB)	// u2 = x2 * z1ˆ2
	ST (u2)
	...
	LDacc (z1in)
	LDt (z2in)
	CALL gmP256MulInternal(SB)	// z1 * z2
	LDt (h)
	CALL gmP256MulInternal(SB)	// z1 * z2 * h
	ST (zout)

	LDacc (hsqr)
	LDt (u1)
	CALL gmP256MulInternal(SB)	// hˆ2 * u1
	ST (u2)
